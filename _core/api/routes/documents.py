...
def _load_google_doc(document_url: str) -> tuple[str, str, str, list[dict]]:
    """Return `(document_id, title, content, sections)` or raise HTTPException."""
    ...
    return document_id, title, content, sections
...
@router.post(
    ...
    description=(
        ...
        "Each chunk stores tab / heading metadata for citations. "
...
async def ingest_google_doc(payload: GoogleDocIngest):
    document_id, title, content, sections = _load_google_doc(payload.document_url)
    try:
...
@router.post(
    ...
    description=(
        ...
        "Chunk metadata includes tab and heading for Source links."
    ),
)
